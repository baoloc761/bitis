  ...
  <section class="py-12 bg-white mt-[70px]">
    <div class="container mx-auto px-4">
      ...
      <Swiper :modules="[Navigation, Autoplay]" :slides-per-view="1" :space-between="20" :loop="true"
        :autoplay="{ delay: 4000, disableOnInteraction: false }" navigation :breakpoints="{
          640: { slidesPerView: 2 },
          ...
          <article
            ...
            <NuxtLink :to="item.link" class="block group relative overflow-hidden">
              <NuxtImg :src="item.image" :alt="item.title"
                ...
                width="400" height="230" format="webp" />
            </NuxtLink>

            <div class="flex-1 flex flex-col justify-between p-8">
              <div>
                <h3 class="text-[15px] font-semibold text-gray-900 leading-snug mb-2 line-clamp-2 uppercase">
                  {{ item.title }}
                </h3>
              ...
              <div
                ...
                <span>{{ item.date }}</span>
                <NuxtLink :to="item.link" class="text-gray-700 font-medium hover:text-blue-600 transition">
                  Xem thêm
                </NuxtLink>
              </div>
            </div>
